/* ═══════════════════════════════════════════════════════════════════
   ...
   Fuentes de datos (mismo contrato que ya documenta split_dataset.py):
     ...
     - lugares-estado.json    → perezoso, solo id + estado_verificacion
                                 (88 KB, ~87% menos que lugares-mapa.json,
                                 el archivo completo de 13 campos que se
                                 pedía antes solo para leer este único
                                 campo). Generado por split_dataset.py a
                                 partir de la misma fuente de siempre
                                 (lugares-mapa.json) — no cambia el dato,
                                 solo el tamaño de lo que viaja por red.
                                 grupo NO se toca desde acá: lugares-mapa.json
                                 sigue desincronizado en ese campo respecto
                                 de lugares-core.json (66 lugares con
                                 "comercios" vs su rubro real), así que
                                 core.json sigue mandando para grupo.
  ...
  function cargarDetallesEnSegundoPlano(){
    var lanzar = function(){
      ...
      fetch('lugares-estado.json')
        ...
        .catch(function(err){ console.warn('lugares-estado.json no disponible', err); });
    };
