/* ═══════════════════════════════════════════════════════════════════
   ...
   1) Mapa-herramienta: interactivo, control real. Aparece SOLO
      cuando la pregunta activa es espacial (Acción Directa, criterio
      de desempate = distancia). Recorte acotado, igual que el resto
      del sistema — nunca el universo completo de puntos.
...
(function (global) {
  ...
  function esConsultaEspacial(consultaTexto) {
    if (!consultaTexto) return false;
    var pistas = ['cerca', 'cerca de mi', 'cerca de mí', 'a metros', 'como llego', 'cómo llego'];
    var q = consultaTexto.toLowerCase();
    return pistas.some(function (p) { return q.indexOf(p) !== -1; });
  }
  ...
  function debeMostrarHerramienta(nombreRegion, consultaTexto) {
    return CFG.mapa.herramientaSoloSiCriterioEspacial &&
      nombreRegion === 'accionDirecta' &&
      esConsultaEspacial(consultaTexto);
  }

  global.URU_MAPA = {
    esConsultaEspacial: esConsultaEspacial,
